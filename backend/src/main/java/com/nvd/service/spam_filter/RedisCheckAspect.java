...
import com.nvd.exceptions.TooManyRequestsException;
...
public class RedisCheckAspect {
    ...
    @Value("${nvd.redis.ttl}")    // thời gian tồn tại của một khiên (60s)
    private Long ttl;

    @Value("${nvd.redis.requests}")    // số reuqest được chạy trong 1 phiên khiên (10)
    ...
    @Around("@annotation(com.nvd.utils.SpamRrequestCheck)")
...
//            return ResponseEntity.status(HttpStatus.TOO_MANY_REQUESTS).body("Too many requests");
            throw new TooManyRequestsException(Constants.TOO_MANY_REQUESTS_EXC);
        }
    }
}
